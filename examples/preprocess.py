...
def gen_data_set_v2(data, seq_max_len=50, negsample=0):
    ...
    train_set = []
    test_set = []
    for reviewerID, hist in tqdm(data.groupby('uid')):
        pos_list = hist['img_id'].tolist()
        # genres_list = hist['genres'].tolist()
        rating_list = hist['event_tag'].tolist()
        ...
        for i in range(1, len(pos_list)):
            ...
            if i != len(pos_list) - 1:
                ...
                test_set.append((reviewerID, pos_list[i], 1, hist[::-1][:seq_len], seq_len,
                                 # genres_hist[::-1][:seq_len],
                                 # genres_list[i],
                                 rating_list[i]))
...
def gen_model_input_v2(train_set, user_profile, seq_max_len):
    train_uid = np.array([line[0] for line in train_set])
    train_iid = np.array([line[1] for line in train_set])
    train_label = np.array([line[2] for line in train_set])
    train_seq = [line[3] for line in train_set]
    ...
    for key in ["serverprovince", "servercity", "city_level", "nettype","model","os","osversion","brand"]:
        train_model_input[key] = user_profile.loc[train_model_input['uid']][key].values
